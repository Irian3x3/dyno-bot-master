...
export default class Premium extends React.Component {
	...
	activateServer = async () => {
		...
		try {
			await axios.post('/account/activate', { serverId: this.state.addServerModal.selectedOption.value });
			this.setState({ addServerModal: { open: false } });
			this.props.getData();
		} catch (err) {
				...
				<div className={'column'}>
					<div className={'subscription premium'}>
						...
						<div className={'active-subscriptions'}>
							<p>You have <strong>{dynoPremiumSubscriptions.length}</strong> Dyno Premium subscription(s), giving you a total of <strong>{dynoPremiumServers}</strong> Dyno Premium server(s).</p>
							{dynoPremiumSubscriptions.map((sub, i) => (<div key={i} className={'active-subscription'}>
								<div className={'subscription-header'}>
									<div className={'title is-5 with-image'}><img className="image is-32x32" src={'https://premium.dyno.gg/images/premium-transparent.png'} /> Dyno {sub.planObject.name}</div>
						...
						<div className={'active-subscriptions'}>
							<p>You have activated <strong>{this.props.braintreeGuilds.length}</strong> out of <strong>{dynoPremiumServers}</strong> Dyno Premium server(s).</p>
							<p>Once you have activated a server, click the button to add the Premium bot to your server.</p>
							{this.props.braintreeGuilds.map((i, k) => (<div key={k} className={'active-subscription'}>
								<div className={'subscription-header'}>
